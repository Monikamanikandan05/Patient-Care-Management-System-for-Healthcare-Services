...
def _render_read_only_prescriptions_table(rx_records, db, is_admin=False):
    """Helper to render structured dataframe table and cards for read-only views."""
    import pandas as pd
    ...
    if is_admin:
        col_config["Patient"] = st.column_config.TextColumn("🧑‍⚕️ Patient", width="medium")
        # Reorder columns so Patient is near start
        cols = ["Patient", "Medication / Tablet", "Dosage", "Frequency", "Timing", "Doctor's Instructions", "Duration", "Prescribing Doctor", "Date Issued"]
        df_rx = df_rx[cols]

    st.dataframe(df_rx, column_config=col_config, use_container_width=True, hide_index=True)
    ...
    for idx, rec in enumerate(rx_records, start=1):
        ...
        card_html   = _render_prescription_card(rec, doc_name, idx)
        ...
        if is_admin:
            ...
            card_html = card_html.replace(
                f"Prescription #{idx}",
                f"Prescription #{idx} &nbsp;&middot;&nbsp; <span style='color:#fff'>Patient: <b>{pat_name}</b></span>"
            )
    ...
    try:
        if user["role"] == "Patient":
            ...
            if rx_records:
                ...
                st.markdown("---")
                _render_read_only_prescriptions_table(rx_records, db, is_admin=False)
            else:
                components.html("""
            ...
            # Admin only gets READ-ONLY access to prescriptions
            st.markdown(
                """<div style="background:linear-gradient(135deg,rgba(168,85,247,.15),rgba(168,85,247,.03));
                    ...
                    <h4 style="margin:0;color:#a855f7;">🛡️ Admin Prescription Management (Read-Only Mode)</h4>
                    <p style="margin:6px 0 0;color:#9ca3af;font-size:.88rem;">
                        Administrators have <b>read-only access</b> to inspect all system-wide prescriptions. 
                        Prescription creation, modification, and deletion operations are strictly restricted to authorized Medical Practitioners (Doctors).
                    </p>
                </div>""",
            ...
            if not rx_records:
                st.info("No prescriptions found in the system.")
                return

            # Search bar
            search_query = st.text_input("🔍 Search Prescriptions", placeholder="Search by patient name, prescribing doctor, or medication...", key="admin_rx_search")
            if search_query:
                filtered_rx = []
            ...
            unique_patients = len(set(r.patient_id for r in rx_records))
            unique_doctors = len(set(r.doctor_id for r in rx_records if r.doctor_id))
            k1.metric("💊 Total Prescriptions", len(rx_records))
            k2.metric("🧑‍⚕️ Unique Patients", unique_patients)
            k3.metric("👨‍⚕️ Prescribing Doctors", unique_doctors)

            st.markdown("---")
            if rx_records:
                _render_read_only_prescriptions_table(rx_records, db, is_admin=True)
            else:
                st.warning("No matching prescriptions found for search query.")

        else:  # Doctor role
            st.markdown("### 👨‍⚕️ Doctor Prescription Workspace")
            doc_tab1, doc_tab2 = st.tabs(["📋 View Prescriptions", "✍️ Issue Prescriptions"])

            with doc_tab1:
                st.markdown("#### 📋 All Issued Prescriptions")
                records = db.query(HealthRecord).order_by(HealthRecord.recorded_at.desc()).all()
                rx_records = [r for r in records if _is_real_prescription(r)]
                if rx_records:
                    _render_read_only_prescriptions_table(rx_records, db, is_admin=True)
            ...
            with doc_tab2:
                ...
                if patients:
                    ...
                    doc_prof = db.query(Doctor).filter(Doctor.user_id == user["id"]).first()
                    ...
                    for idx in range(st.session_state.pv_num_rx_items):
                        with st.expander(f"💊 Tablet #{idx + 1} Details", expanded=True):
                            ...
                            with rx_col1:
                                medication = st.text_input(f"Medication Name & Strength #{idx+1}", placeholder="e.g. Amoxicillin 500mg", key=f"pv_med_{idx}")
