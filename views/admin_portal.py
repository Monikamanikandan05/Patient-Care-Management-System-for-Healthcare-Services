...
from views.prescriptions_view import _render_prescription_card, _is_real_prescription
...
def render_admin_portal():
    ...
    try:
        ...
        with tab7:
            ...
            if rx_records:
                st.markdown(f"**{len(rx_records)} prescription(s) issued globally.**")
                all_cards_html = """
                <style>
                ...
                for idx, rec in enumerate(rx_records, start=1):
                    ...
                    # Also include patient info in the card for admin view
                    pat = db.query(User).filter(User.id == rec.patient_id).first()
                    pat_name = pat.full_name if pat else "Unknown Patient"
                    
                    # We inject the patient name into the generated card
                    card = _render_prescription_card(rec, doc_name, idx)
                    # Simple string replacement to insert patient name
                    card = card.replace(
                        f"Prescription #{idx}", 
                        f"Prescription #{idx} &nbsp;·&nbsp; <span style='color:#fff'>For: {pat_name}</span>"
                    )
                    all_cards_html += card

                card_height = 160 + len(rx_records) * 145
                components.html(all_cards_html, height=card_height, scrolling=True)
            else:
